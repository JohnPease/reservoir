...
| Accent surface | `ReservoirSurfaceAccent` | `#C7EEE4` | `#123847` | Credit transaction icon bg, goal-met completion banner (active tab bar pill usage descoped, see §3.6) |
...
### 3.6 Tab bar
- Active tab ("Today"): icon + label tinted `ReservoirAccent`, no background.
- Inactive tabs: icon + label tinted `ReservoirTextMuted`, no background.
- **Descoped (reservoir-d2s):** the original spec called for the active tab to sit on a pill-shaped `ReservoirSurfaceAccent` background. Stock SwiftUI `TabView` (pre-Liquid-Glass tab bar) has no public API for a per-item selection background — only `.tint()` (selected item) and `UITabBar.appearance().unselectedItemTintColor` (inactive items), both already applied in `RootTabView.swift`. Achieving the pill exactly as specified would require replacing `TabView` with a fully custom bottom tab bar (hand-rolled selection state, safe-area handling, and accessibility tab traits that `TabView` provides for free) — assessed at ~120–180 LOC / ~4–6 hours, disproportionate for a 2-tab personal-use app with no external users. Product-lead and engineering assessed the tradeoff and JP confirmed tint-only is the final treatment; the pill background is won't-fix.
